...
module.exports = function PostGraphileNestedMutationPlugin(
  ...
  builder.hook('GraphQLInputObjectType:fields', (fields, build, context) => {
    ...
    const {
      scope: {
 isInputType, isPgRowType, isPgPatch, pgIntrospection: table,
},
    } = context;
     ...
     * whitelist
     */
    if (nestedMutationsTableNameWhiteList && !nestedMutationsTableNameWhiteList[table.name]) return fields;
    ...
    pgNestedPluginForwardInputTypes[table.id].forEach(({ name, keys, connectorInputField }) => {
      ...
      nestedFields[name] = Object.assign({}, fields[name], { type: connectorInputField });
    });
      ...
      await Promise.all(
        nestedFields
          ...
          .map(async (nestedField) => {
            const {
 constraint, foreignTable, keys, foreignKeys, name: fieldName,
} = nestedField;
      ...
      try {
        ...
        if (isPgCreateMutationField) {
          ...
          if (isPgNodeMutation) {
            ...
            condition = sql.fragment`(${sql.join(
              keys.map(
                key => sql.fragment`${sql.identifier(key.name)} = ${gql2pg(
        ...
        await Promise.all(
          Object.keys(inputData).map(async (key) => {
            ...
            if (fieldValue.deleteOthers) {
              ...
              const { text: deleteQueryText, values: deleteQueryValues } = sql.compile(deleteQuery);
